...
保留 1 Core + 1 项目状态模板 + 5 Skills 架构，附 Codex 项目级模型配置。这是供 Agent 读取的工作规范，不含自动调度、权限执行或上下文压缩程序。
...
模型分工及升级条件统一见 [AGENTS.md 的模型分工](AGENTS.md#模型分工)：总控和困难任务使用高档，普通执行子 Agent 使用中档。无需为五个 Skill 分别创建 Agent 配置。
